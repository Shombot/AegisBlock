...
public class ZeroKnowledgeThreshold extends ZKPProtocol {
	private static HashMap<int[], MatrixInterface> history = new HashMap<int[], MatrixInterface>();
	...
	protected ArrayList<BigInteger> internalNullChallenges(CryptoData response, BigInteger challenge, ArrayList<BigInteger> list) {
		...
		if(c.length != p.length) {
			CryptoData[] coefficients = c;
			BigInteger[] challenges = new BigInteger[p.length];
			for(int i = 0; i < p.length; i++) {
				challenges[i] = challenge;
				for(int j = 0; j < coefficients.length; j++) {
					challenges[i] = challenges[i].add(coefficients[j].getBigInt().multiply(BigInteger.valueOf(i+1).modPow(BigInteger.valueOf(j+1), challengePrime))).mod(challengePrime);
				}
			}
			c = new CryptoDataArray(challenges).getCryptoDataArray();
		}
		for(int i = 0; i < p.length; i++) {
			if(z[i] == null) {
...
//			System.out.println(i[i.length - 1]);
		...
		for(int j = 0; j < o.length; j++)
		{
			...
			if(c != null) 
			{
				if(i[j] == null)
					...
					o[j] = p[j].initialCommSim(i[j], c.getBigInt(), environment[j]);
				fakeProofFound++;
			}
			else
			{
	...
	public CryptoData initialCommSim(CryptoData input, BigInteger challenge, CryptoData packedEnvironment) throws MultipleTrueProofException, ArraySizesDoNotMatchException, NoTrueProofException {
		...
		BigInteger[] values = new BigInteger[p.length+10];
		...
		for(int i = 0; i < challenges.length; i++) {
			challenges[i] = challenge;
			for(int j = 0; j < coefficients.length;j++) {
				challenges[i] = challenges[i].add(coefficientsBigInt[j].multiply((BigInteger.valueOf(i+1)).modPow(BigInteger.valueOf(j+1), challengePrime))).mod(challengePrime);
			}
		}
	...
	public CryptoData calcResponse(CryptoData input, BigInteger challenge, CryptoData packedEnvironment) throws NoTrueProofException, MultipleTrueProofException {
		...
		if(m == null){
			...
			for(int i = 0; i < numSimulated; i++) {
				basic[i+1][0] = BigInteger.ONE;
				for(int j = 1; j < numSimulated+1; j++) {
					basic[i+1][j] = BigInteger.valueOf(positions[i]+1).modPow(BigInteger.valueOf(j), challengePrime);
				}
			}
		...
		MatrixInterface simChallengesM = new Matrix(simChallenges, challengePrime);
		MatrixInterface coefficientsM = simChallengesM.multiply(m);
		BigInteger[][] coefficients = coefficientsM.getMatrix();
		...
		for(int i = 0; i < p.length; i++) {
			...
			if(count != positions.length && positions[count] == i) {
				...
				for(int j = 0; j < numSimulated; j++) {
					c = c.add(coefficients[j+1][0].multiply(BigInteger.valueOf(i+1).modPow(BigInteger.valueOf(j+1), challengePrime))).mod(challengePrime);
					
				}
				toReturn[i] = p[i].calcResponse(in[i], c, environment[i]);
			}
		}
	...
	public boolean verifyResponse(CryptoData input, CryptoData a_unopened, CryptoData z_unopened, BigInteger challenge, CryptoData environments) {
		...
		{ 
			...
			for(int j = 0; j != coefficients.length; j++) {
				c = c.add(coefficients[j].getBigInt().multiply(BigInteger.valueOf(i+1).modPow(BigInteger.valueOf(j+1), challengePrime))).mod(challengePrime);

			}
			flag = p[i].verifyResponse(in[i], a[i], z[i], c, e[i]);
			if(!flag) 
			{
				System.out.println("Challenge " + i + " = " + c);
	...
	{
		String toReturn = "PT"+(p.length - numSimulated) +"-of-"+p.length+"(";
		for(int i = 0; i < p.length; i++)
		{
			if(i != 0) toReturn += ", ";
...
//					System.out.println("sim " + i + " " + c);					
	...
	public CryptoData initialCommSim(CryptoData publicInput, CryptoData secrets, BigInteger challenge,
		...
		CryptoData[] s = secrets.getCryptoDataArray();
		CryptoData[] out = new CryptoData[p.length];
		CryptoData[] coefficients = s[p.length].getCryptoDataArray();
		try {
			if(coefficients.length != numSimulated) throw new ArraySizesDoNotMatchException("" + numSimulated + " != " + coefficients.length);
		}catch(NullPointerException e)
		{
		...
		BigInteger[] challenges = new BigInteger[p.length];
		BigInteger[] coefficientsBigInt = new BigInteger[coefficients.length];
		BigInteger[] values = new BigInteger[p.length+1];
		for(int i = 0; i < values.length; i++) {
			values[i] = BigInteger.valueOf(i);
		}
		...
		for(int j = 0; j < p.length; j++)
		{
			...
			if(in[j] == null) 
				...
				out[j] = p[j].initialCommSim(in[j], s[j], c, env[j]);
		}
		return new CryptoDataArray(out);
...
//		return new CryptoDataArray(out);
	...
	public CryptoData calcResponse(CryptoData publicInput, CryptoData secrets, BigInteger challenge,
		...
		MatrixInterface coefficientM = new Matrix(simChallenges, challengePrime).multiply(m);
		BigInteger[][] coefficients = coefficientM.getMatrix();
...
//		System.out.println("positions = " + Arrays.toString(positions));
		...
		for(int i = 0; i < p.length; i++) {
			if(count < positions.length && positions[count] == i) {
				...
				toReturn[i] = p[i].calcResponse(in[i], s[i], c, e[i]);
			}
		}
...
//		System.out.println(Arrays.toString(coefficientsToReturn));
		toReturn[p.length] = new CryptoDataArray(coefficientsToReturn);
		return new CryptoDataArray(toReturn);
	}
//TODO
