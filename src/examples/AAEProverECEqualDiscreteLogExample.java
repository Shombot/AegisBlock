...
public class AAEProverECEqualDiscreteLogExample {
	public static void main(String[] args) throws IOException, ClassNotFoundException, MultipleTrueProofException, NoTrueProofException, ArraySizesDoNotMatchException {
		...
		ZKPProtocol proof = new ECEqualDiscreteLogsProver();
		/*
		...
		CryptoData publicInputs;
		{
			...
			CryptoData[] inner = new CryptoData[] {new ECPointData(gx), new ECPointData(hx)};
		...
		CryptoData secrets;
		{
			CryptoData[] inner = new CryptoData[] {new BigIntData(ZKToolkit.random(order, rand)), new BigIntData(x)};
			secrets = new CryptoDataArray(inner);
		}
		//Create Environment
		CryptoData env;	
		{
