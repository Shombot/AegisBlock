...
public class AAFProverProofOfZeroorOneORExample {
	public static void main(String[] args) throws IOException, ClassNotFoundException, MultipleTrueProofException, NoTrueProofException, ArraySizesDoNotMatchException {
		...
		out.writeObject(ciphertext[1].getEncoded(true));
		out.flush();
		
		//ephemeral = ephemeral.add(BigInteger.ONE);
		ZKPProtocol proof;
		{
		...
		//sec = [rp, x]
		//env = [g, h]
		CryptoData secrets = null;
		
		if(message.equals(BigInteger.ZERO)) {
			
			BigInteger r1 = ZKToolkit.random(order, rand);
			BigInteger r2 = ZKToolkit.random(order, rand);
			BigInteger[] inner1 = new BigInteger[] {r1, ephemeral};
			BigInteger[] inner2 = new BigInteger[] {r2};
			BigInteger[] simChal = new BigInteger[] {null, ZKToolkit.random(order, rand)};
			...
			BigInteger[] inner2 = new BigInteger[] {r2, ephemeral};
